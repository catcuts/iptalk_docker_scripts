...
(function() {
    ...
    b.prototype.open = function() {  // MarkerTool 的打开方法
        if (!this._map) {
            return false
        }
        ...
        if (!this._followMarker) {  // TODO_1: 在这里将 _followMarker 改为自定义 customerMarker, 继承自 customerOverlay
    ...
    // TODO_3: 增加一个 b.prototype.setDOM
    ;
    ...
    b.prototype._bind = function() {
        ...
        c._mouseMoveHandler = function(d) {  // 处理鼠标移动(放置时的移动，非拖动移动，拖动由 Marker 类内置的方法处理)
            var e = d.point;
            c._followMarker.setIcon(c._opts.icon);  // TODO_2: 改成调用 setDOM
        ...
        c._clickHandler = function(d) {  // 处理鼠标点击(放置点击，非拖动前的点击)
            var i = d.pixel;
