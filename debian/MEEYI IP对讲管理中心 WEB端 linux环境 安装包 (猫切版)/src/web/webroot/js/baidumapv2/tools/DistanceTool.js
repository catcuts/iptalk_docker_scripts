...
(function() {
     ...
     */
    DistanceTool.prototype.open = function(offsetTop){
        // 判断测距状态是否已经开启
        ...
        var distMove = function(e) {
            ...
            if (me._paths.length > 0) {
                ...
                var y = me._bind.y + curY - me._bind.initY// + 46;
        ...
        // 绑定全部事件
        baidu.on(OperationMask.getDom(this._map), "click", distClick);  // 点击以确定起点
        baidu.on(document, "mousemove", distMove);  // 移动测距/选择起点
        baidu.on(OperationMask.getDom(this._map), "dblclick", distDblclick);  // 双击以确定重点
        baidu.on(document, "keydown", distKeyDown);  // 按键以取消
        baidu.on(OperationMask.getDom(this._map), "mouseup", distMouseUp);  // 鼠标放开以结束本段测距，进入下一段测距
    ...
    var OperationMask = {
         ...
         * @type {String}
         */
        _html : "<div style='background:transparent url(http://api.map.baidu.com/images/blank.gif);position:absolute;left:0;top:-46px;width:100%;height:100%;z-index:1000' unselectable='on'></div>",
