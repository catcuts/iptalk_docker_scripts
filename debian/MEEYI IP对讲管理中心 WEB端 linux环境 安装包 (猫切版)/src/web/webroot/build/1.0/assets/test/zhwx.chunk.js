...
/***/ "./node_modules/babel-loader/lib/index.js!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue":
/***/ (function(module, exports, __webpack_require__) {
...
__webpack_require__("./src/assets/js/semantic/components/image.min.css");

__webpack_require__("./src/assets/js/semantic/components/list.min.css");

__webpack_require__("./src/assets/js/semantic/components/card.min.css");
...
String.prototype.contains = function (keyword) {
    keyword = keyword.replace(/\s{2,}/, " ").split(" ");
    console.log("keyword:", keyword);
    for (i = 0; i < keyword.length; i++) {
        if (!this.toString().includes(keyword[i])) {
...
String.prototype.discontains = function (keyword) {
    keyword = keyword.replace(/\s{2,}/, " ").split(" ");
    console.log("keyword:", keyword);
    for (i = 0; i < keyword.length; i++) {
        if (this.toString().includes(keyword[i])) {
...
exports.default = {
    components: {
        menubar: _menubar2.default,
        icon: _icon2.default,
        richform: _form2.default
    },
    methods: {
        onBodyDblclick: function onBodyDblclick($event) {},
        onBodyClick: function onBodyClick($event) {
            var _this = this;

            var listItem = this.getClickItem($event.target);
            if (listItem !== undefined) {
                var selPk = $(listItem).attr("pk");
                var addItems = [];
                if (this.list.multiSelect) {
                    if ($event.which == 1) {
                        if ($event.ctrlKey) {
                            addItems.push(selPk);
                        } else if ($event.shiftKey) {
                            var endIndex = this.getListItemIndex(listItem);
                            var startIndex = -1;
                            if (this.selected.length > 0) {
                                startIndex = this.getListItemIndex(this.selected[0]);
                            }
                            if (startIndex > -1 && endIndex > -1) {
                                ...
                                addItems.push.apply(addItems, _toConsumableArray(this.listData.map(function (item, index) {
                                    return String(item[_this.list.dataKey] ? item[_this.list.dataKey] : index);
                ...
                this.addToSelected(addItems);
            }
        },
        addToSelected: function addToSelected(pks) {
            var _this2 = this;

            if (typeof pks == "string") {
                if (this.selected.indexOf(pks) == -1) {
                    this.selected.push(pks);
                }
            } else if ($.isArray(pks)) {
                pks.forEach(function (pk) {
                    if (_this2.selected.indexOf(pk) == -1) {
                        _this2.selected.push(pk);
                    }
                });
            } else {
                logger.warn("错误的列表项主健。");
            }
        },
        getClickItem: function getClickItem(ele) {
            var $Ele = $(ele);
            if ($Ele) {
                if ($Ele.hasClass("listItem")) {
                    return $Ele.get(0);
                } else {
                    var $PEles = $Ele.parents(".listItem");
                    if ($PEles.length > 0) {
                        return $PEles.get(0);
                    } else {
                        return undefined;
        ...
        getListItemIndex: function getListItemIndex(itemEleOrPk) {
            try {
                if ((typeof itemEleOrPk === "undefined" ? "undefined" : _typeof(itemEleOrPk)) == "object") {
                    return $(this.$el).children(".richlist-body").children(".listItem[pk=" + $(itemEleOrPk).attr("pk") + "]").index();
                } else {
                    return $(this.$el).children(".richlist-body").children(".listItem[pk=" + itemEleOrPk + "]").index();
                }
            } catch (e) {
                return -1;
            }
        },
        getListItemByPrimary: function getListItemByPrimary(pk) {
            return $(this.$el).children(".richlist-body").children(".listItem[pk=" + pk + "]").eq(0);
        },
        getDefaultOf: function getDefaultOf() {
            var _this3 = this;
            ...
            for (var _len = arguments.length, metaNames = Array(_len), _key = 0; _key < _len; _key++) {
                metaNames[_key] = arguments[_key];
            }

            metaNames.forEach(function (metaName, metaNameIndex) {
                switch (metaName) {
                    case "listItem":
                        metaDefaultArr.push({
                            iconSize: "huge",
                            colNames: _this3.getDefaultOf("colNames"),
                        ...
                        break;
                    case "colNames":
                        metaDefaultArr.push([L("第一列"), L("第二列"), L("第三列")]);}
                var metaNameObj = {};
                metaNameObj[metaName + "Default"] = metaDefaultArr[metaNameIndex];
                metaDefaultObj = Object.assign(metaDefaultObj, metaNameObj);
            });
            if (metaNames.length == 1) {
                return metaDefaultObj[metaNames[0] + "Default"];
            }
            return metaDefaultOjb;
        },
        onRefresh: function onRefresh() {
            logger.warning("onRefresh");
        },
        onFilter: function onFilter() {
            logger.debug("onFilter");
            this.list.filter.enabled = !this.list.filter.enabled;
            if (this.meta.filter === undefined) {
                this.$set(this.meta, "filter", this.list.filter);
            } else {
                this.meta.filter = this.list.filter;
            }
        },
        onSort: function onSort() {
            var orders = ["desc", "asc"];
            this.list.sort.order = this.list.sort.order === "asc" ? "desc" : orders[1 + orders.indexOf(this.list.sort.order)];
            if (this.meta.sort === undefined) {
                this.$set(this.meta, "sort", this.list.sort);
            } else {
                this.meta.sort = this.list.sort;
            }

            logger.warning("onSort: " + this.list.sort.order);
        },
        switchToIconView: function switchToIconView() {
            logger.warning("switchToIconView");
            this.list.view = "icon";
            if (this.meta.view === undefined) {
                this.$set(this.meta, "view", this.list.view);
            } else {
                this.meta.view = this.list.view;
            }
        },
        switchToListView: function switchToListView() {
            logger.warning("switchToListView");
            this.list.view = "list";
            if (this.meta.view === undefined) {
                this.$set(this.meta, "view", this.list.view);
            } else {
        ...
        switchToCardView: function switchToCardView() {
            logger.warning("switchToCardView");
            this.list.view = "card";
            if (this.meta.view === undefined) {
                this.$set(this.meta, "view", this.list.view);
            } else {
        ...
        getListData: function getListData(options) {},
        sortListData: function sortListData(sortConfig, sortData) {
            var dataNumSorted = [];
            var dataStrSorted = [];
            var dataNotSorted = [];

            sortData.forEach(function (data) {
                if (typeof data[sortConfig.key] === "number") {
                    dataNumSorted.push(data);
                } else if (typeof data[sortConfig.key] === "string") {
                    dataStrSorted.push(data);
                } else {
                    dataNotSorted.push(data);
                }
            });

            dataNumSorted.sort(function (a, b) {
                return (sortConfig.order === "desc" ? -1 : sortConfig.order === "asc" ? 1 : 1) * (a[sortConfig.key] - b[sortConfig.key]);
            });

            dataStrSorted.sort(function (a, b) {
                return (sortConfig.order === "desc" ? -1 : sortConfig.order === "asc" ? 1 : 1) * a[sortConfig.key].localeCompare(b[sortConfig.key]);
            });

            return dataNumSorted.concat(dataStrSorted.concat(dataNotSorted));
        },
        typeOf: function typeOf(value) {
            switch (typeof value === "undefined" ? "undefined" : _typeof(value)) {
                case "number":
        ...
        checkArgsType: function checkArgsType(args, argsType) {
            var _this4 = this;

            var typeError = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "函数参数错误";
            var systemError = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "函数参数异常";

            Object.keys(argsType).forEach(function (arg, argIndex) {
                try {
                    Object.keys(args[argIndex]).forEach(function (key) {
                        if (Array.isArray(argsType[arg][key]) && argsType[arg][key].indexOf(_typeof(args[argIndex][key])) === -1 || !Array.isArray(argsType[arg][key]) && _this4.typeOf(args[argIndex][key]) !== argsType[arg][key]) {
                            logger.error(typeError + ": " + key + "(" + _typeof(args[argIndex][key]) + "类型)" + "应为 " + argsType[arg][key] + "类型");
                            return;
                        }
                    });
                } catch (e) {
                    logger.error(systemError + ": " + key + "\n\t" + e);
                    return;
                }
            });
        },
        ...
        pageNumberChange: function pageNumberChange() {
            logger.debug("pageNumberChange" + this.list.footer.items[2].text);
            var page = parseInt(this.list.footer.items[2].text.replace(/\/.*$/, ""));
            if (page < 1) {
                this.page = 1;
    ...
    data: function data() {
        return {
            defaultToolbar: [{ name: "refresh", text: "", icon: "refresh", tips: "刷新", right: true, click: "onRefresh" }, { name: "filter", text: "过滤", icon: "filter", tips: "过滤", right: true, click: "onFilter" }, { name: "sort", text: "", icon: "sort", tips: "排序", right: true, click: "onSort" }, { name: "switchToIconView", text: "", icon: "image", tips: "图标", right: true, click: "switchToIconView" }, { name: "switchToListView", text: "", icon: "list layout", tips: "列表", right: true, click: "switchToListView" }, { name: "switchToCardView", text: "", icon: "block layout", tips: "卡片", right: true, click: "switchToCardView" }],
            defaultFooter: [{ name: "first", click: "firstPage", icon: "angle double left", tips: "首页" }, { name: "previous", click: "previousPage", icon: "angle left", tips: "上一页" }, { name: "pagenumber", type: "textbox", text: "1/1", align: "center", width: 80, expandWidth: 120, change: "pageNumberChange" }, { name: "next", click: "nextPage", icon: "angle right", tips: "下一页" }, { name: "last", click: "lastPage", icon: "angle double right", tips: "最后一页" }, { icon: "refresh", click: "onRefresh", showText: false, right: true }],
            page: 1,
            totalPage: 1
        };
    },
    ...
    computed: {
        list: function list() {
            var listItemDefault = this.getDefaultOf("listItem");
            var listMeta = (0, _deepAssign2.default)({
                id: this.generateUUID(),
                title: "",
                description: "",
                url: "",
                        ...
                        fields: [{
                            ...
                            attrs: {
                                inlineItems: [{
                                    label: "",
                                    attrs: {
                                        leftItems: [{ text: "包含", value: "contains" }, { text: "不包含", value: "discontains" }]
                                    }
                                }, {
                                    label: "",
                                    attrs: {
                                        leftItems: [{ text: "开头是", value: "startsWidth" }, { text: "结尾是", value: "endsWidth" }]
                                    }
                                }]
                            }
                        }],
            ...
            if (listMeta.toolbar.default) {
                var _listMeta$toolbar$ite;

                (_listMeta$toolbar$ite = listMeta.toolbar.items).push.apply(_listMeta$toolbar$ite, _toConsumableArray(this.defaultToolbar));
            }
            if (listMeta.footer.default) {
                var _listMeta$footer$item;

                (_listMeta$footer$item = listMeta.footer.items).push.apply(_listMeta$footer$item, _toConsumableArray(this.defaultFooter));
            }
            return listMeta;
        },

        hasHeader: function hasHeader() {
            return this.list.header.visible == undefined ? true : this.list.header.visible;
        },

        hasFilter: function hasFilter() {
            return this.list.filter.visible == undefined ? true : this.list.filter.visible;
        },

        hasToolbar: function hasToolbar() {
            return this.list.toolbar.visible == undefined ? true : this.list.toolbar.visible;
        },

        hasFooter: function hasFooter() {
            return this.list.footer.visible == undefined ? true : this.list.footer.visible;
        },

        listItemAttrs: function listItemAttrs() {
            return (0, _deepAssign2.default)(this.list.listItem, this.list.views[this.list.view].listItem);
        },

        processedListData: function processedListData() {
            if (this.list.dataMode.toLowerCase() === "ajax") {
                webservice.getJSON(url, encodeURL(JONS.stringify({ filterRule: this.list.filter.rule[0], sortConfig: this.list.sort })));
                var listDataFiltered = this.listData;
            } else {
                var rules = this.list.filter.rule[0];
                var ruleArr = [];
                for (var rule in rules) {
                    if (Array.isArray(rules[rule])) {
                        var subRuleArr = [];
                        rules[rule].forEach(function (r) {
                            if (/(\d{4}-\d{2}-\d{2})/.test(r)) {
                                var ruleEle = "new Date(data['" + rule + "'])" + r.replace(/(\d{4}-\d{2}-\d{2})/, function (r0, p1) {
                                    return new Date(p1).getTime();
                                });
                            } else {
                                var ruleEle = "data['" + rule + "']" + r;
                            }
                            subRuleArr.push(ruleEle);
                        });
                        if (subRuleArr.length) {
                            rules[rule] = "(" + subRuleArr.join("||") + ")";
                            ruleArr.push(rules[rule]);
                        }
                    } else {
                        var operators = ["contains", "discontains", "startsWith", "endsWith"];
                        operators.forEach(function (operator) {
                            if (rules[rule].startsWith(operator)) {
                                rules[rule] = "." + operator + "('" + rules[rule].replace(new RegExp(operator + '\\s*'), "") + "')";
                            }
                        });

                        var subRules = rules[rule].split("&&");
                        subRules.forEach(function (subRule) {
                            if (subRule) {
                                if (/(\d{4}-\d{2}-\d{2})/.test(subRule)) {
                                    var ruleEle = "new Date(data['" + rule + "'])" + subRule.replace(/(\d{4}-\d{2}-\d{2})/, function (r, p1) {
                                        return new Date(p1).getTime();
                                    });
                                } else {
                                    var ruleEle = "data['" + rule + "']" + subRule;
                                }
                                ruleArr.push(ruleEle);
                            }
                        });
                    }
                }
                ...
                logger.debug("ruleStr:" + ruleStr);

                if (ruleStr) {
                    var listDataFiltered = this.listData.filter(function (data) {
                        try {
                            var result = eval(ruleStr);
                        } catch (e) {
                            var result = false;
                        }
                        return result;
                ...
                if (this.list.sort.order != "none") {
                    listDataFiltered = this.sortListData(this.list.sort, listDataFiltered);
                }

                var pageSize = this.listItemAttrs.pageSize;

                if (pageSize >= 0) {
                    this.totalPage = Math.ceil(listDataFiltered.length / pageSize);
                } else {
                    this.totalPage = 1;
                    pageSize = listDataFiltered.length;
                }
                this.list.footer.items[2].text = this.page + "/" + this.totalPage;

                listDataFiltered = listDataFiltered.slice(pageSize * (this.page - 1), pageSize * this.page);
            }
    ...
    props: {
        meta: { type: Object, default: function _default() {} },
        listData: { type: Array, default: function _default() {
                return [];
            } },
    ...
    created: function created() {
        var self = this;
        this.list.toolbar.items.forEach(function (item) {
            if (typeof item.click != "function" && item.click && self[item.click]) {
                self.$on(item.click, self[item.click]);
            }
        });

        this.list.footer.items.forEach(function (item) {
            if (typeof item.click != "function" && item.click && self[item.click]) {
                self.$on(item.click, self[item.click]);
            } else if (typeof item.change != "function" && item.change && self[item.change]) {
                self.$on(item.change, self[item.change]);
            }
        });
    }
};
...
/***/ "./node_modules/babel-loader/lib/index.js!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./utils/lang-loader.js!./test/zhwx/app.vue":
/***/ (function(module, exports, __webpack_require__) {
...
var _list = __webpack_require__("./src/components/richlist/list.vue");
...
exports.default = {
  components: { menubar: _menubar2.default, tabs: _tabs2.default, layout: _layout2.default, gridLayout: _gridLayout2.default, zwxtable: _table2.default, richform: _form2.default, RichButton: _button2.default, RichInput: _input2.default, richlist: _list2.default },
  mixins: [_loadingMixin2.default, _dnd2.default],
  name: "testapp",
  data: function data() {
    return {
      testtabs: [{ tab: "tabaa", text: "方向", icon: "save" }, { tab: "tabbb", text: "保存", icon: "print", label: "2" }, { tab: "tabcc", text: "表单", active: true, icon: "save" }],
      formdata: [{
        name: "张三丰",
        address: "福建省泉州市通源街东方花园城",
        phone: "22182666",
        sex: true,
        active: false,
        email: "dds@125.com",
      ...
      inputMeta2: {
        icon: "edit",
        actions: [{ name: "name", text: "选择", icon: "save" }, { name: "address", icon: "edit", text: "保存" }],
        actionsPos: "right"
      },
      inputMeta3: {
        label: "http://",
      ...
      listMeta: {},
      listData: [{ id: 1, text: "张三丰", icon: "edit", tips: "提示信息" }, { id: 2, text: "王二是", icon: "save", tips: "提示信息" }, { id: 3, text: "百度中蝇中是是中", icon: "edit", tips: "提示信息" }, { id: 4, text: "蝇中中中需中虽", icon: "edit", tips: "提示信息" }, { id: 5, text: "张三丰", icon: "home", tips: "提示信息" }, { id: 6, text: "张三丰", icon: "edit", tips: "提示信息" }, { id: 7, text: "王二是", icon: "edit", tips: "提示信息" }, { id: 8, text: "百度中蝇中是是中", icon: "edit", tips: "提示信息" }, { id: 9, text: "蝇中中中需中虽", icon: "edit", tips: "提示信息" }, { id: 10, text: "张三丰", icon: "user", tips: "提示信息" }, { id: 11, text: "张三丰", icon: "edit", tips: "提示信息" }, { id: 12, text: "王二是", icon: "edit", tips: "提示信息" }, { id: 13, text: "百度中蝇中是是中", icon: "edit", tips: "提示信息" }, { id: 14, text: "蝇中中中需中虽", icon: "edit", tips: "提示信息" }, { id: 15, text: "张三丰", icon: "edit", tips: "提示信息" }],

      formMeta: {
        url: "/test/zhwx/api/testform.js",
        errorShowIn: "top",
        celled: true,
        inline: true,
        fit: false,
        ...
        fields: [[{ name: "name", type: "text", labelWidth: 100, labelAlign: "right", width: 6, label: "姓名", default: "默认值1", required: true, placeholder: "姓名", help: "html文本" }, { name: "email", icon: "edit", labelWidth: 100, width: 10, inlineWidth: 120, validation: "email", type: "textpro", label: "邮件地址", default: "默认值2", placeholder: "地址", help: "html文本" }], { name: "name", type: "text", inlineWidth: 100, width: 6, label: "姓名", default: "默认值1", required: true, placeholder: "姓名", help: "html文本" }, { name: "email", icon: "edit", validation: "email", type: "textpro", width: 6, label: "邮件地址", default: "默认值2", placeholder: "地址", help: "html文本" }, { name: "active", icon: "edit", type: "checkbox", width: 6, label: "激活", attrs: {} }, [{ name: "name2", type: "text", width: 6, label: "姓名2", default: "默认值1", required: true, placeholder: "姓名", help: "html文本" }, { name: "address3", icon: "edit", type: "textpro", width: 6, label: "地址2", default: "默认值2", placeholder: "地址", help: "html文本",
          attrs: {
            actions: [{ name: "name", text: "选择", icon: "save", click: function click(btn, e) {
                alert("选择");
              } }, { name: "address", icon: "edit", text: "保存" }]
          }
        }], {
          name: "christmas",
          type: "tabs",
          ...
          tabs: [{
            ...
            tab: "global",
            active: true,
            fields: [{ name: "memo", type: "text", validation: "email", width: 6, label: "备注", default: "默认值1", required: true, placeholder: "姓名", help: "html文本" }, { name: "card", type: "text", width: 6, label: "卡号", default: "默认值2", placeholder: "地址", help: "html文本" }]
          }, {
            text: "高级",
            tab: "adv",
            fields: [{ name: "sex", type: "text", width: 6, label: "全称", default: "默认值1", required: true, placeholder: "姓名", help: "html文本" }, { name: "cardNo", type: "text", width: 6, label: "身份证号码", default: "默认值2", placeholder: "地址", help: "html文本" }]
          }]
        }, { name: "phon4e", type: "text", width: 6, label: "电话", default: "默认值3", placeholder: "电话", help: "html文本" }, { name: "phone33", type: "text", width: 6, label: "电话", default: "默认值3", placeholder: "电话", help: "html文本" }]
      },
      tableMenu: [{ text: "刷新" }, { text: "分组框" }, { text: "标题" }, { text: "页脚" }, { type: "textbox", text: "2/12", icon: "print", width: 160, expandWidth: 350 }],
      eventTableCols: [{ name: "name", "title": "事件", width: "180px", align: "center" }, { name: "params", sort: true, "title": "参数" }],
      events: [],
      selectedRows: [3, 5],
      tableCols: [{ name: "enabled", "title": "允许", type: "checkbox", width: "100px", align: "center" }, { name: "id", "title": "编号", width: "80px", align: "center", sortable: true }, { name: "name", "title": "姓名", width: "180px", sortable: true,
        formatter: function formatter(value, row, col) {
          return "<i class='print icon'></i>" + value;
        } }, { name: "sex", "title": "性别", width: "160px", sortable: true, type: "switch",
        formatter: { icons: { default: 'print', 1: 'remove', 2: 'home' }, type: "icon" }
      }, { name: "address", "title": "地址",
        buttons: [{ icon: "print", class: "positive", click: function click(row, col, event) {
            alert("buttons click:", row.id, col.name);
          } }, { icon: "remove", class: "basic" }], type: "button", align: "center", sortable: true }, { name: "birday", "title": "生日", width: "100px",
        formatter: { template: "生日:{value}", type: "template" }
      }, { name: "post", "title": "职位", width: "100px" }],
      tableRows: [{ id: 111, name: "张三1", sex: 1, address: "dsdfsdfsd", expand: false, birday: "1922-12-1", post: "总经理1" }, { id: 112, name: "dsf2", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理2" }, { id: 113, name: "sdfds3", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1921-12-3", post: "总经理3" }, { id: 1114, name: "4dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "323-24-21", post: "总经理4" }, { id: 5, name: "5sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "323-12-2", post: "总经理" }, { id: 6, name: "6张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "323-12-2", post: "总经理" }, { id: 7, name: "7dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "323-12-2", post: "总经理" }, { id: 8, name: "8sdfds", sex: 2, address: "dsdfsdfsd", expand: true, birday: "1922-12-2", post: "总经理" }, { id: 9, name: "9张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "3232-12-2", post: "总经理" }, { id: 10, name: "10dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 11, name: "11sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 12, name: "12张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 13, name: "13dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 14, name: "14sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 15, name: "15张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 16, name: "16 dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 17, name: "17 sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 18, name: "18 张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 19, name: "19 dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 20, name: "20 sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 21, name: "21 张三", sex: 1, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 22, name: "22 dsf", sex: 0, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }, { id: 23, name: "23 sdfds", sex: 2, address: "dsdfsdfsd", expand: false, birday: "1922-12-2", post: "总经理" }],
      draggable: {
        selector: ["#testdrag>li"]
      },
      loading: {
        selector: "[slot='taba']"
      },
      tabitems: [{ tab: "taba", text: "方向", icon: "save" }, { tab: "tabb", text: "保存", active: true, icon: "print", label: "2" }, { tab: "tabc", text: "表单", icon: "save" }, { text: "方向", type: "search", options: {
          source: [{ title: "aaaaa" }, { title: "bbbbb" }, { title: "cccc" }] } }, { id: "bug", text: "方向", type: "menu", showArrow: false, showText: false, icon: "bug", children: [{ text: "方向", icon: "save" }, { text: "方向", icon: "save" }] }, { id: "testtext", text: "", type: "textbox", icon: "print" }, { id: "label", text: "搜索", type: "button", click: "onSearch" }, { id: "label2", text: "增加", type: "checkbox", change: "onChangeChecked" }],
      items: [{ id: "a33", text: "垂直方向", click: "onChangeDirection", icon: "home", right: true }, { id: "VueA", text: "VueA", vue: "devices/operates/safebox.operate.vue", click: 'onLoadVue', el: $("#vue1").get(0), icon: "folder open outline" }, { id: "VueB", text: "VueB", vue: "devices/operates/iptalk.host.operate.vue", click: 'onLoadVue', el: $("#vue2").get(0), icon: "folder open outline" }, { id: "c3", text: "对话框", icon: "hyt ico", click: "onSave", ui: "./a.vue" }, { id: "d3gf", text: "弹出", icon: "meeyi ico", click: "onPopupDialog" }, { id: "alarm", type: "menu", text: "告警", icon: "trash", children: [{ id: "alarmd", icon: "save", text: "发现人" }, { id: "label", text: "搜索", type: "checkbox", change: "onChangeChecked" }, { id: "alarmsubmenu", icon: "bug", text: "发现车", children: [{ id: "e123", text: "发现事件1" }, { id: "label", text: "搜索", type: "checkbox", change: "onChangeChecked" }, { id: "e12", text: "发现事件2", click: "onFindEvents" }, { id: "e133", text: "发现事件3" }] }, { text: "-" }, { id: "e", text: "发现事件" }] }, { id: "d322", text: "加粗", group: "a" }, { id: "d32", text: "打印", click: "onChangeLayout" }, { id: "edf", text: "工具", type: "toolbar", icon: "", children: [{ id: "d", icon: "save", text: "发现人" }, { id: "d", text: "发现车" }, { id: "e", text: "发现事件" }] }, { id: "f3f", text: "编辑", color: "red" }, { id: "g2", text: "项目" }, { id: "a2", text: "张立", icon: "home", right: true }, { id: "bbb", text: "定制", type: "popup", popupPos: "bottom left", icon: "save", right: true }, { id: "a2", text: "测试", icon: "home", children: [{ id: "d", icon: "save", text: "发现人" }, { id: "d", text: "发现车" }, { id: "e", text: "发现事件" }], right: true }, { id: "a1", text: "AA", icon: "home", click: "onA1", right: true }],
      dialogOptions: {
        title: "美一对讲",
        fit: false,
        maxable: true,
        ...
        dimmer: false,
        type: "vue",
        source: "devices/operates/safebox.operate.vue",
        autoDestroy: true
      },
      PopupDialogOptions: {
        title: "美一对讲",
      ...
      testValues: { test: "张三丰", abs: { name: "李四娘" } }
    };
  },

  methods: {
    onBtnClick: function onBtnClick(btn, $event) {
      alert("点击了:" + btn.text);
    },
    onInputAction: function onInputAction(action, $event) {
      alert("点击了:" + actionbtn.text);
    },
    ...
    onSetDragData: function onSetDragData(event) {
      ...
      event.dataTransfer.setData("text", $ele.attr("data"));
    },
    onLoadingCancel: function onLoadingCancel(target) {},
    onFormAction: function onFormAction(action) {
      console.log(this, action);
    },
    onLoadingRetry: function onLoadingRetry(target) {
      alert("retry111!");
    },
    onHeaderClick: function onHeaderClick(col) {
      this.events.push({ name: "HeaderClick", params: col.name });
    },
    onCellClick: function onCellClick(row, col) {
      this.events.push({ name: "CellClick", params: col.name });
    },
    onRowClick: function onRowClick(row) {
      this.events.push({ name: "RowClick", params: row });
    },
    onRowExpand: function onRowExpand(row, target) {
      console.log("RowExpand=", target);
      this.events.push({ name: "RowExpand", params: JSON.stringify(row) });
    },
    onLoadRowDetial: function onLoadRowDetial(td, pk) {
      console.log("onLoadRowDetial=", td, pk);
    },
    onColChecked: function onColChecked(col) {
      this.events.push({ name: "ColChecked", params: col.name });
    },
    onLoadData: function onLoadData(context) {
      this.events.push({
        name: "onLoadData",
        params: "curPage:" + context.curPage + ",pageCount=" + context.pageCount + ",pageSize=" + context.pageSize
      });
      var self = this;
      var newPage = context.newPage;
      setTimeout(function () {
        $.getJSON("/test/tabledata", function (data) {
          try {
            context.updateRows(data.result.records);
            context.success(newPage);
          } catch (e) {
            context.fail("数据格式不匹配错误!");
          }
        }).fail(function (e) {
          context.fail(e.responseText);
        });
  ...
  created: function created() {
    this.registerEvents({
      onPopupDialog: function onPopupDialog(item) {
        if (!this.PopupDialog) {
          this.PopupDialog = new _dialog.PopupDialog(this.PopupDialogOptions);
        }
        this.PopupDialog.show($(".item[title=VueB]"));
      },
      onA1: function onA1(item) {
        alert("请注意我们的提醒消息");
      },
      onFindEvents: function onFindEvents(item) {
        prompt("请输入您的姓名：", "张三丰", "aaa").then(function (value) {
          alert(value);
        });
      },
      onLoadVue: function onLoadVue(item) {},
      onChangeLayout: function onChangeLayout(item) {
        var glayout = this.$refs.glayout;
        glayout.preset = glayout.preset + 1;
      },
      onLoading: function onLoading(item) {
        this.showLoading();
      },
      onChangeChecked: function onChangeChecked(item) {
        var mainmenu = this.$refs.mainmenu;
        this.items.push({ id: parseInt(Math.random() * 100), text: "aaa", icon: "print" });
      },
      onTextChange: function onTextChange(item) {
        console.log("onTextChange=", item.text);
      },
      onSearch: function onSearch(item) {
        this.items.push({ id: parseInt(Math.random() * 100), text: "aaa", icon: "print" });
      },
      onChangeDirection: function onChangeDirection(item) {
        alert("onChangeDirection");
        var mainmenu = this.$refs.mainmenu;

        mainmenu.addMenuItem("alarmsubmenu", { text: "新菜单" }, 0);
      },
      onSave: function onSave(item) {
        var dialog = new _dialog.RichDialog(this.dialogOptions, this.testValues);
      }
    });
  },
  mounted: function mounted() {}
...
/***/ "./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./test/zhwx/app.vue":
/***/ (function(module, exports, __webpack_require__) {
...
/***/ "./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue":
/***/ (function(module, exports, __webpack_require__) {
...
// module
exports.push([module.i, "\n.richlist[data-v-5957729d] {\n    /* 高度自适应 */\n    position: relative;\n    min-height: 100%;\n    height: 100%;\n}\n.richlist.fit[data-v-5957729d] {\n    position: absolute;\n    top: 0px;\n    left: 0px;\n    width: 100%;\n    height: 100%;\n}\n.richlist.hasborder[data-v-5957729d] {\n    border: 1px solid #DEDEDE;\n}\n.richlist.hasborder>.ui.attached.segment[data-v-5957729d] {\n    border: none;\n    width: 100%;\n    margin-left: 0px;\n    margin-right: 0px;\n}\n.richlist.hasborder>.richlist-toolbar.menubar.ui.menu[data-v-5957729d] {\n    border-left: none;\n    border-right: none;\n}\n.richlist>.richlist-body[data-v-5957729d] {\n    position: relative;\n    /*min-height: 200px;*/\n    display:flex;\n    flex-wrap:wrap;\n    align-items: flex-start;\n    align-content: flex-start;\n    flex-basis:600px;\n\n    /* 高度自适应 */\n    height: calc(100% - 60px);\n    overflow-y: scroll;\n}\n.richlist>.richlist-body>.listItem[data-v-5957729d]{\n    position: relative;\n    cursor:pointer;\n    font-size:1em;\n}\n\n/* 漂浮状态 */\n.richlist>.richlist-body>.icon.listItem[data-v-5957729d]:hover,\n.richlist>.richlist-body>.item.listItem[data-v-5957729d]:hover,\n.richlist>.richlist-body>.card.listItem[data-v-5957729d]:hover{\n    outline:1px solid #2185D0;\n}\n\n/* 图标视图 */\n.richlist>.richlist-body>.icon.listItem[data-v-5957729d]{\n    min-width:100px;\n    text-align:center;\n    padding:0.1em;\n    padding-bottom:2em;\n}\n\n/* 列表视图 */\n.richlist>.richlist-body>.item.listItem[data-v-5957729d]{\n    width:inherit;\n    /*border: 1px solid #2e8cd2;\n    /*margin: 0 auto 0 auto*/\n}\n.richlist>.richlist-body>.card.listItem[data-v-5957729d]:first-child{\n    margin-top: auto;\n}\n.richlist>.richlist-body>.card.listItem[data-v-5957729d]:last-child{\n    margin-bottom: auto;\n}\n\n/* 选中状态 */\n.richlist>.richlist-body>.icon.listItem.selected[data-v-5957729d],\n.richlist>.richlist-body>.item.listItem.selected[data-v-5957729d],\n.richlist>.richlist-body>.card.listItem.selected[data-v-5957729d]{\n    background: #2e8cd2;\n    color: white;\n}\n.richlist>.richlist-body>.icon.listItem>i[data-v-5957729d]{\n    margin:0px;\n}\n.richlist>.richlist-body>.icon.listItem>span[data-v-5957729d]{\n    position:absolute;\n    left:0px;\n    bottom:0px;\n    right:0px;\n    width:100%;\n    padding:2px;\n    height:2em;\n    white-space: nowrap;\n    text-overflow: ellipsis;\n    overflow: hidden;\n}\n.richlist .fade-enter-active[data-v-5957729d], .richlist .fade-leave-active[data-v-5957729d] {\n    transition: opacity .5s\n}\n.richlist .fade-enter[data-v-5957729d], .richlist .fade-leave-active[data-v-5957729d] {\n    opacity: 0\n}\n.richform>.richform-body .field[data-v-5957729d] {\n    padding-left: 10px !important;\n    padding-right: 10px !important;\n}\n.richlist>.richlist-footer[data-v-5957729d] {\n    /* 高度自适应 */\n    position: absolute;\n    bottom: 0;\n    width: 100%;\n}\n\n", ""]);
...
/***/ "./node_modules/css-loader/index.js!./src/assets/js/semantic/components/card.min.css":
/***/ (function(module, exports, __webpack_require__) {
...
/***/ "./node_modules/css-loader/index.js!./src/assets/js/semantic/components/image.min.css":
/***/ (function(module, exports, __webpack_require__) {
...
/***/ "./node_modules/vue-loader/lib/template-compiler/index.js?{\"id\":\"data-v-01611136\",\"hasScoped\":true}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./utils/lang-loader.js!./test/zhwx/app.vue":
/***/ (function(module, exports, __webpack_require__) {

module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: "zhwx_testapp"
  }, [_vm._m(0), _vm._v(" "), _c('menubar', {
    ref: "mainmenu",
    attrs: {
      "shadow": false,
      "inverted": false,
    ...
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.formMeta.header.visible),
      expression: "formMeta.header.visible"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.formMeta.header.visible) ? _vm._i(_vm.formMeta.header.visible, null) > -1 : (_vm.formMeta.header.visible)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.formMeta.header.visible,
          $$el = $event.target,
        ...
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.formMeta.header.visible = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.formMeta.header.visible = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.formMeta.header.visible = $$c
        }
      }
    }
  }), _vm._v("header")]), _vm._v(" "), _c('span', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.formMeta.toolbar.visible),
      expression: "formMeta.toolbar.visible"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.formMeta.toolbar.visible) ? _vm._i(_vm.formMeta.toolbar.visible, null) > -1 : (_vm.formMeta.toolbar.visible)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.formMeta.toolbar.visible,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.formMeta.toolbar.visible = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.formMeta.toolbar.visible = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.formMeta.toolbar.visible = $$c
        }
      }
    }
  }), _vm._v("toolbar")]), _vm._v(" "), _c('span', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.formMeta.footer.visible),
      expression: "formMeta.footer.visible"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.formMeta.footer.visible) ? _vm._i(_vm.formMeta.footer.visible, null) > -1 : (_vm.formMeta.footer.visible)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.formMeta.footer.visible,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.formMeta.footer.visible = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.formMeta.footer.visible = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.formMeta.footer.visible = $$c
        }
      }
    }
  }), _vm._v("footer")]), _vm._v(" "), _c('span', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.formMeta.fit),
      expression: "formMeta.fit"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.formMeta.fit) ? _vm._i(_vm.formMeta.fit, null) > -1 : (_vm.formMeta.fit)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.formMeta.fit,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.formMeta.fit = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.formMeta.fit = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.formMeta.fit = $$c
        }
  ...
  }), _vm._v("fit")]), _vm._v(" "), _c('div', [_vm._v("\n          " + _vm._s(_vm.formdata[0]) + "\n        ")]), _vm._v(" "), _c('div', {
    staticStyle: {
      "margin-left": "50px",
      "width": "800px",
      "padding": "20px",
    ...
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.title),
      expression: "dialogOptions.title"
    }],
    domProps: {
      "value": (_vm.dialogOptions.title)
    },
    on: {
      "input": function($event) {
        ...
        _vm.dialogOptions.title = $event.target.value
      }
    }
    ...
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.moveable),
      expression: "dialogOptions.moveable"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.dialogOptions.moveable) ? _vm._i(_vm.dialogOptions.moveable, null) > -1 : (_vm.dialogOptions.moveable)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.dialogOptions.moveable,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.dialogOptions.moveable = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.dialogOptions.moveable = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.dialogOptions.moveable = $$c
        }
      }
    }
  }), _vm._v("移动")]), _vm._v(" "), _c('p', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.resizable),
      expression: "dialogOptions.resizable"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.dialogOptions.resizable) ? _vm._i(_vm.dialogOptions.resizable, null) > -1 : (_vm.dialogOptions.resizable)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.dialogOptions.resizable,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.dialogOptions.resizable = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.dialogOptions.resizable = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.dialogOptions.resizable = $$c
        }
      }
    }
  }), _vm._v("缩放")]), _vm._v(" "), _c('p', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.maximize),
      expression: "dialogOptions.maximize"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.dialogOptions.maximize) ? _vm._i(_vm.dialogOptions.maximize, null) > -1 : (_vm.dialogOptions.maximize)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.dialogOptions.maximize,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.dialogOptions.maximize = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.dialogOptions.maximize = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.dialogOptions.maximize = $$c
        }
      }
    }
  }), _vm._v("最大化")]), _vm._v(" "), _c('p', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.minimize),
      expression: "dialogOptions.minimize"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.dialogOptions.minimize) ? _vm._i(_vm.dialogOptions.minimize, null) > -1 : (_vm.dialogOptions.minimize)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.dialogOptions.minimize,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.dialogOptions.minimize = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.dialogOptions.minimize = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.dialogOptions.minimize = $$c
        }
      }
    }
  }), _vm._v("最小化")]), _vm._v(" "), _c('p', [_c('input', {
    directives: [{
      name: "model",
      rawName: "v-model",
      value: (_vm.dialogOptions.closeable),
      expression: "dialogOptions.closeable"
    }],
    attrs: {
      "type": "checkbox"
    },
    domProps: {
      "checked": Array.isArray(_vm.dialogOptions.closeable) ? _vm._i(_vm.dialogOptions.closeable, null) > -1 : (_vm.dialogOptions.closeable)
    },
    on: {
      "__c": function($event) {
        var $$a = _vm.dialogOptions.closeable,
          $$el = $event.target,
          $$c = $$el.checked ? (true) : (false);
        if (Array.isArray($$a)) {
          var $$v = null,
            $$i = _vm._i($$a, $$v);
          if ($$el.checked) {
            $$i < 0 && (_vm.dialogOptions.closeable = $$a.concat([$$v]))
          } else {
            $$i > -1 && (_vm.dialogOptions.closeable = $$a.slice(0, $$i).concat($$a.slice($$i + 1)))
          }
        } else {
          _vm.dialogOptions.closeable = $$c
        }
      }
    }
    ...
    slot: "R2C1"
  }, [_vm._v("sssssss")]), _vm._v(" "), _c('div', {
    attrs: {
      "slot": "R2C2"
    },
    slot: "R2C2"
  }, [_c('button', [_vm._v("dsdsd")])])])], 1), _vm._v(" "), _c('div', {
    attrs: {
      "slot": "last"
    },
    slot: "last"
    ...
    staticClass: "hyt ico24 "
  }), _vm._v(" "), _c('i', {
    staticClass: "hyt ico32 "
  }), _vm._v(" "), _c('i', {
    staticClass: "meeyi ico"
  }), _vm._v(" "), _c('i', {
    staticClass: "meeyi ico24 "
  }), _vm._v(" "), _c('ul', {
    attrs: {
      "id": "testdrag"
    }
  ...
  }, [_vm._v("aaaaaaaaaaaaa")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "http://mockjs.com"
    }
  }, [_vm._v("网站")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "{\"type\":\"image\",\"url\":\"http://www.qzwb.com/gb/img/9264.files/zhitongche.jpg\",\"params\":{}}"
    }
  }, [_vm._v("图片1")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "http://www.qzwb.com/gb/img/9264.files/zhitongche.jpg"
    }
  }, [_vm._v("图片2")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "{\"type\":\"vue\",\"url\":\"a.vue\",\"params\":{}}"
    }
  }, [_vm._v("VUE组件a")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "{\"type\":\"vue\",\"url\":\"b.vue\",\"params\":{}}"
    }
  }, [_vm._v("VUE组件b")]), _vm._v(" "), _c('li', {
    attrs: {
      "data": "www.mp4"
    }
    ...
    attrs: {
      ...
      "paging": true,
      "row-formatter": _vm.onDynamicRowStyle,
      "pagination": {
        pageCount: 5,
        pageNumber: 1,
        pageSize: 10
      },
      "show-detial-row": true,
      "multi-select": true
    },
    on: {
      "loadrowdetial": _vm.onLoadRowDetial,
      "rowexpand": _vm.onRowExpand,
      "headerclick": _vm.onHeaderClick,
      "rowclick": _vm.onRowClick,
      "cellclick": _vm.onCellClick,
      "colchecked": _vm.onColChecked,
      "loaddata": _vm.onLoadData
    },
    scopedSlots: _vm._u([{
      key: "rowdetial",
      fn: function(props) {
        return [_vm._v("\n                        " + _vm._s(props.row.name) + "-----" + _vm._s(props.row.post) + "\n                      ")]
      }
    }])
  ...
  }, [_vm._v("ass"), _c('p', [_vm._v("ds")]), _c('p', [_vm._v("ds")]), _c('p', [_vm._v("ds")]), _c('p', [_vm._v("ds")]), _c('p', [_vm._v("dsdssssssss")]), _c('p', [_vm._v("ds")])])
}]}
module.exports.render._withStripped = true
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-01611136", module.exports)
  }
}
...
/***/ "./node_modules/vue-loader/lib/template-compiler/index.js?{\"id\":\"data-v-5957729d\",\"hasScoped\":true}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue":
/***/ (function(module, exports, __webpack_require__) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    class: [
      'richlist',
      _vm.list.border ? 'hasborder' : '',
      _vm.list.compact ? 'compact' : '',
      _vm.list.fit ? 'fit' : ''
    ],
    attrs: {
      "id": _vm.list.id
    ...
    style: (_vm.list.header.style)
  }, [_c('h2', {
    staticClass: "ui blue header"
  }, [(_vm.list.header.icon) ? _c('i', {
    staticClass: "icon",
    class: _vm.list.header.icon
  }) : _vm._e(), _vm._v(" "), _c('div', {
    staticClass: "content"
  }, [_vm._v("\n                " + _vm._s(_vm.list.header.title) + "\n                "), (_vm.list.header.description) ? _c('div', {
    staticClass: "sub header"
  }, [_vm._v(_vm._s(_vm.list.header.description))]) : _vm._e()])])]) : _vm._e()]), _vm._v(" "), _vm._t("toolbar", [(_vm.hasToolbar) ? _c('menubar', {
    class: [
      'richlist-toolbar',
    ...
    attrs: {
      "theme": _vm.theme,
      "items": _vm.list.toolbar.items,
      "attached": _vm.top
    }
  }) : _vm._e()]), _vm._v(" "), _vm._t("filter", [_c('transition', {
    attrs: {
      "name": "fade"
    }
  }, [(_vm.list.filter.enabled) ? _c('richform', {
    attrs: {
      "meta": _vm.list.filter.meta,
      "form-data": _vm.list.filter.rule
    },
    on: {
      "fieldValueSet": _vm.filterChange
    }
  }) : _vm._e()], 1)]), _vm._v(" "), _c('div', {
    staticClass: "richlist-body ui attached segment",
    style: (_vm.list.style),
    on: {
      "click": function($event) {
        _vm.onBodyClick($event)
      },
      "dblclcik": function($event) {
        _vm.onBodyDblclick($event)
      }
    }
  }, _vm._l((_vm.processedListData), function(listItem, listItemIndex) {
    return _c('div', {
      key: listItem[_vm.list.dataKey] || '',
      class: [
        _vm.listItemAttrs.viewClass,
        _vm.list.view,
        _vm.selected.indexOf(String(listItem[_vm.list.dataKey])) == -1 ? '' : _vm.listItemAttrs.selectedClass,
        'listItem'
      ],
      style: (_vm.listItemAttrs.viewStyle),
      attrs: {
        "pk": listItem[_vm.list.dataKey] || 'row_' + listItemIndex,
        "title": listItem.tips
      }
    }, [(_vm.list.view == 'icon') ? [_c('icon', {
      attrs: {
        "name": listItem.icon,
        "size": _vm.listItemAttrs.iconSize
      }
    }), _vm._v(" "), _c('span', [_vm._v(_vm._s(listItem.text))])] : (_vm.list.view == 'list') ? [_c('div', {
      ...
      staticClass: "content"
    }, [_vm._v("\n                    " + _vm._s(listItem.text) + "\n                ")])] : [_vm._t(_vm.list.view, null, {
      listItem: listItem,
      listItemAttrs: _vm.listItemAttrs,
      listItemIndex: listItemIndex
    })]], 2)
  })), _vm._v(" "), _vm._t("footer", [(_vm.hasFooter) ? _c('menubar', {
    ref: "footer",
    staticClass: "richlist-footer",
    attrs: {
      "items": _vm.list.footer.items,
      "docked": "bottom"
    }
  }) : _vm._e()])], 2)
},staticRenderFns: []}
...
if (false) {
  module.hot.accept()
  if (module.hot.data) {
     require("vue-hot-reload-api").rerender("data-v-5957729d", module.exports)
  }
}
...
/***/ "./node_modules/vue-style-loader/index.js!./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./test/zhwx/app.vue":
/***/ (function(module, exports, __webpack_require__) {

// style-loader: Adds some css to the DOM by adding a <style> tag

// load the styles
var content = __webpack_require__("./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./test/zhwx/app.vue");
if(typeof content === 'string') content = [[module.i, content, '']];
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__("./node_modules/vue-style-loader/lib/addStylesClient.js")("6e1b7a88", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
   module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!../../utils/lang-loader.js!./app.vue", function() {
     var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!../../utils/lang-loader.js!./app.vue");
...
/***/ "./node_modules/vue-style-loader/index.js!./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue":
/***/ (function(module, exports, __webpack_require__) {

// style-loader: Adds some css to the DOM by adding a <style> tag

// load the styles
var content = __webpack_require__("./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue");
if(typeof content === 'string') content = [[module.i, content, '']];
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__("./node_modules/vue-style-loader/lib/addStylesClient.js")("9cae956c", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
   module.hot.accept("!!../../../node_modules/css-loader/index.js!../../../node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!../../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!../../../utils/lang-loader.js!./list.vue", function() {
     var newContent = require("!!../../../node_modules/css-loader/index.js!../../../node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!../../../node_modules/vue-loader/lib/selector.js?type=styles&index=0!../../../utils/lang-loader.js!./list.vue");
     if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
     update(newContent);
   });
 }
...
/***/ "./src/assets/js/semantic/components/card.min.css":
/***/ (function(module, exports, __webpack_require__) {

// style-loader: Adds some css to the DOM by adding a <style> tag

// load the styles
var content = __webpack_require__("./node_modules/css-loader/index.js!./src/assets/js/semantic/components/card.min.css");
if(typeof content === 'string') content = [[module.i, content, '']];
...
if(false) {
	// When the styles change, update the <style> tags
	if(!content.locals) {
		module.hot.accept("!!../../../../../node_modules/css-loader/index.js!./card.min.css", function() {
			var newContent = require("!!../../../../../node_modules/css-loader/index.js!./card.min.css");
			if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
			update(newContent);
		});
	}
...
/***/ "./src/assets/js/semantic/components/image.min.css":
/***/ (function(module, exports, __webpack_require__) {

// style-loader: Adds some css to the DOM by adding a <style> tag

// load the styles
var content = __webpack_require__("./node_modules/css-loader/index.js!./src/assets/js/semantic/components/image.min.css");
if(typeof content === 'string') content = [[module.i, content, '']];
// Prepare cssTransformation
var transform;
...
if(false) {
	// When the styles change, update the <style> tags
	if(!content.locals) {
		module.hot.accept("!!../../../../../node_modules/css-loader/index.js!./image.min.css", function() {
			var newContent = require("!!../../../../../node_modules/css-loader/index.js!./image.min.css");
			if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
			update(newContent);
		});
	}
...
/***/ "./src/components/richlist/list.vue":
/***/ (function(module, exports, __webpack_require__) {

var disposed = false
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__("./node_modules/vue-style-loader/index.js!./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-5957729d\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue")
}
var Component = __webpack_require__("./node_modules/vue-loader/lib/component-normalizer.js")(
  /* script */
  __webpack_require__("./node_modules/babel-loader/lib/index.js!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue"),
  /* template */
  __webpack_require__("./node_modules/vue-loader/lib/template-compiler/index.js?{\"id\":\"data-v-5957729d\",\"hasScoped\":true}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./utils/lang-loader.js!./src/components/richlist/list.vue"),
  /* styles */
  injectStyle,
  /* scopeId */
  "data-v-5957729d",
...
Component.options.__file = "D:\\project\\webfrontend\\stwebfrontend\\src\\components\\richlist\\list.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
...
if (false) {(function () {
  ...
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-5957729d", Component.options)
  } else {
    hotAPI.reload("data-v-5957729d", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
...
function injectStyle (ssrContext) {
  if (disposed) return
  __webpack_require__("./node_modules/vue-style-loader/index.js!./node_modules/css-loader/index.js!./node_modules/vue-loader/lib/style-compiler/index.js?{\"vue\":true,\"id\":\"data-v-01611136\",\"scoped\":true,\"hasInlineConfig\":false}!./node_modules/vue-loader/lib/selector.js?type=styles&index=0!./utils/lang-loader.js!./test/zhwx/app.vue")
}
var Component = __webpack_require__("./node_modules/vue-loader/lib/component-normalizer.js")(
  /* script */
  __webpack_require__("./node_modules/babel-loader/lib/index.js!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./utils/lang-loader.js!./test/zhwx/app.vue"),
  /* template */
  __webpack_require__("./node_modules/vue-loader/lib/template-compiler/index.js?{\"id\":\"data-v-01611136\",\"hasScoped\":true}!./node_modules/vue-loader/lib/selector.js?type=template&index=0!./utils/lang-loader.js!./test/zhwx/app.vue"),
  /* styles */
  injectStyle,
  /* scopeId */
  "data-v-01611136",
...
Component.options.__file = "D:\\project\\webfrontend\\stwebfrontend\\test\\zhwx\\app.vue"
if (Component.esModule && Object.keys(Component.esModule).some(function (key) {return key !== "default" && key.substr(0, 2) !== "__"})) {console.error("named exports are not supported in *.vue files.")}
...
if (false) {(function () {
  ...
  module.hot.accept()
  if (!module.hot.data) {
    hotAPI.createRecord("data-v-01611136", Component.options)
  } else {
    hotAPI.reload("data-v-01611136", Component.options)
  }
  module.hot.dispose(function (data) {
    disposed = true
  })
